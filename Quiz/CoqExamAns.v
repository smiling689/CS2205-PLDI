...
Proof.
  ...
  - simpl in H. 
    right.
    exists lu; split; auto.
  - destruct lu. 
    * simpl in H. 
      left. 
      exists (a :: ly); split; auto.
    * inversion H; subst a0; clear H. 
      apply IHly in H2 as [ [? []] | [? []] ]; subst. 
      ** left. exists x; split; auto.
      ** right. exists x; split; auto.
  (* 另一个归纳的方向，处理append会稍微麻烦一点*)
  (* induction lx; intros.
  - rewrite app_nil_r in H.
    ...
    * rewrite app_nil_r. 
      reflexivity.
  - assert ((ly ++ a :: nil) ++ lx = lu ++ lv). { 
      rewrite <- app_assoc.
      simpl. apply H. }
    pose proof (IHlx (ly ++ (cons a nil)) lu lv H0) as [ [? []]| [? []] ]; subst. 
    * clear - H1. 
      assert (x = @nil A \/ x <> @nil A) by tauto.
      destruct H.
      ** subst. right.
         rewrite app_nil_r in H1. 
         exists (cons a nil); split; auto.
      ** apply exists_last in H as [? []]; subst.
...
Proof.
  intros l1 l2 l3 [lx [ly [H1 H2]]] [lu [lv [H3 H4]]]; subst.
  pose proof (app_split3 lx ly lu lv H3) as [[? []] | [? []]]; subst; clear.
  - exists (lx ++ lu), x; split; rewrite app_assoc; reflexivity.
  - exists x, (lv ++ ly); split; rewrite app_assoc; reflexivity.
Qed.
...
Proof.
  ...
  - rewrite Rels_concat_indexed_union_distr_r.
    ...
    rewrite <- (Sets_included_indexed_union (n + m)%nat).
    rewrite power_rel_add.
    reflexivity.
  - apply Sets_indexed_union_included. intros n.
    rewrite <- (Sets_included_indexed_union n) at 1.
    rewrite <- (Sets_included_indexed_union O).
    simpl. rewrite Rels_concat_id_r.
    reflexivity.
Qed.
...
Proof.
  intros.
  sets_unfold; simpl. 
  intros; split; simpl; intros [? [ [] [] ] ]; subst.
  - exists a0; split; split; auto.
  - exists a0; split; split; auto.
Qed.
...
Proof.
  intros; unfold cequiv; simpl; unfold if_sem.
  repeat rewrite Rels_concat_union_distr_l.
  unfold test_true, test_false.
  assert (forall X (A B C D: X -> X -> Prop), A ∪ B ∪ (C ∪ D) == (A ∪ C) ∪ (B ∪ D)) by firstorder. 
  rewrite H.
  apply Sets_union_congr; apply Sets_union_congr;
  ( do 2 rewrite <- Rels_concat_assoc; 
    apply Rels_concat_congr;
    try reflexivity;
    apply Rels_test_concat_comm).
Qed.
