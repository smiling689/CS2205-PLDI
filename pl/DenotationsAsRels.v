...
(** 两种定义方式的区别在于：第一种定义方式是通过无限次迭代来定义的，而第二种定义方式是通过有界迭代来定义的。
    具体而言，第一种定义方式中的_[iterLB]_表示执行循环体任意次后退出循环的行为，而第二种定义方式中的_[boundedLB]_表示执行循环体不超过_n_次后退出循环的行为。
    由于循环体可以执行任意次，因此两种定义方式实际上是等价的。*)
