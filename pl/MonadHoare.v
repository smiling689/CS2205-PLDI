...
(** 下面这条性质刻画了幂运算的幂运算的性质。请同学们试着完成证明，提示：证明中可
    能会用到_[Nat.add_comm]_。*)
...
Import AssocUnit FastPower.
Section fast_power_correct.
...
Proof.
  ...
  apply (Hoare_repeat_break _
           (fun '(exp', base', res') =>
              res' * power base' exp' = power base exp)).
  + intros [ [exp' base'] res'].
    intros.
    ...
    - apply Hoare_assume_bind.
      ...
      rewrite H0 in H.
      simpl in H.
      rewrite <- H, right_unit.
      reflexivity.
    - apply Hoare_assume_bind.
      intros.
      apply Hoare_ret.
      apply Nat_Even_fact in H0.
      rewrite <- H.
      rewrite H0 at 2.
      rewrite power_2.
      rewrite power_power.
